...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($dtb < 1) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-massege alert-danger" role="alert">Keranjang Kosong</div>');
            redirect(base_url());
        } else {
    ...
    public function tambahBooking()
    {
        ...
        if ($databooking > 0) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Paket Anda sedang Dikirim.<br> Tunggu 1x24 Jam untuk Belanja Kembali </div>');
            redirect(base_url());
        }

        if ($temp > 0) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Produk ini Sudah ada Dikeranjang </div>');
            redirect(base_url() . 'home');
        }

        if ($tempuser == 3) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Produk Dikeranjang Maksimal 3</div>');
            redirect(base_url() . 'home');
        }
    ...
    public function hapusbooking()
    {
        ...
        if ($kosong < 1) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-massege alert-danger" role="alert">Semua Produk Dikeranjang Telah Dihapus</div>');
            redirect(base_url());
        } else {
